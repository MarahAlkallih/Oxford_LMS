...
import { useAddSessionFileMutation } from "../../../../services/sessions/files/sessionFiles";
...
export const AddSessionFileModal = ({ open, onClose, sessionId}: AddFileModalProps) => {
  const [file, setFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [isDragging, setIsDragging] = useState(false);

  const [addSessionFile, { isLoading }] = useAddSessionFileMutation();
  ...
  const handleSubmit = async () => {
    ...
    formData.append("courseId", sessionId.toString());
  ...
  return (
    <Modal open={open} onClose={handleClose}>
      <div className="flex flex-col gap-5 p-6 min-w-85 sm:min-w-105">
