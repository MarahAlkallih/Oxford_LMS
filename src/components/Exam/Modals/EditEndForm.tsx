...
import { useEditEndFormMutation } from "../../../services/exams/forms/endFormMutation";

import { useGetEndFormByIdQuery } from "../../../services/exams/forms/endFormQuery";

import { toast } from "react-toastify";
import { CheckBox } from "../../Fields/CheackBox";

interface EditEndFormModalProps {
  open: boolean;
  onClose: () => void;
  id: number;
...
export const EditEndFormModal = ({ open, onClose, id }: EditEndFormModalProps) => {

  const { data, isFetching } = useGetEndFormByIdQuery({ id }, { skip: !open || !id });

  const [editForm, { isLoading: isUpdating }] = useEditEndFormMutation();
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div className="flex flex-col gap-4 p-4 min-w-87.5">
        <h2 className="text-xl font-semibold text-center">Edit End Form</h2>
